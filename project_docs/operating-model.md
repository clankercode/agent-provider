...
6. **End or failure.** Completion closes the stream. Before dispatch,
   navigation, cancellation, timeout, or bridge loss prevents external work.
   After dispatch, cancellation is best effort: the runtime signals abort,
   reports `cancel-requested` or `outcome-unknown` when completion cannot be
   proven, and never replays uncertain provider requests or callbacks.
   Consequential callbacks must use the supplied idempotency key and provide
   application-specific reconciliation when their outcome is unknown.
...
V1 application origins are an exact build-time allowlist that produces exact
content-script matches. Deployment eligibility and the user's grant are
separate checks. A persistent grant binds the exact origin, allowed aliases,
alias fingerprints, least-strict permitted base mode, scope, and expiry;
remapping an alias or broadening scope requires extension reconfirmation.
