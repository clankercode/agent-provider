...
A grant records the aliases approved during consent, fingerprints their
provider/model mappings, and records the least-strict base mode permitted.
Switching to audit-first is always allowed because it tightens approval;
switching to standard, adding an alias, changing an alias mapping, extending
expiry, or otherwise broadening scope requires extension-owned reconfirmation.
...
V1 persistent retention defaults are 30 days, 10,000 events, and 10 MiB
globally, with oldest-first deletion at the first reached limit. Users may
tighten these limits. Export, managed enterprise logging, cross-device
synchronization, and higher caps remain future decisions.
